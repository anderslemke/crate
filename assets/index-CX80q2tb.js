 ...
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */(function(e){function t(x,j){var z=x.length;x.push(j);e:for(;0<z;){var J=z-1>>>1,ne=x[J];if(0<o(ne,j))x[J]=j,x[z]=ne,z=J;else break e}}function n(x){return x.length===0?null:x[0]}function r(x){if(x.length===0)return null;var j=x[0],z=x.pop();if(z!==j){x[0]=z;e:for(var J=0,ne=x.length,gr=ne>>>1;J<gr;){var It=2*(J+1)-1,Vo=x[It],Pt=It+1,wr=x[Pt];if(0>o(Vo,z))Pt<ne&&0>o(wr,Vo)?(x[J]=wr,x[Pt]=z,J=Pt):(x[J]=Vo,x[It]=z,J=It);else if(Pt<ne&&0>o(wr,z))x[J]=wr,x[Pt]=z,J=Pt;else break e}}return j}function o(x,j){var z=x.sortIndex-j.sortIndex;return z!==0?z:x.id-j.id}if(typeof performance=="object"&&typeof performance.now=="function"){var i=performance;e.unstable_now=function(){return i.now()}}else{var l=Date,u=l.now();e.unstable_now=function(){return l.now()-u}}var a=[],c=[],p=1,m=null,h=3,y=!1,w=!1,S=!1,O=typeof setTimeout=="function"?setTimeout:null,f=typeof clearTimeout=="function"?clearTimeout:null,s=typeof setImmediate<"u"?setImmediate:null;typeof navigator<"u"&&navigator.scheduling!==void 0&&navigator.scheduling.isInputPending!==void 0&&navigator.scheduling.isInputPending.bind(navigator.scheduling);function d(x){for(var j=n(c);j!==null;){if(j.callback===null)r(c);else if(j.startTime<=x)r(c),j.sortIndex=j.expirationTime,t(a,j);else break;j=n(c)}}function v(x){if(S=!1,d(x),!w)if(n(a)!==null)w=!0,lt(k);else{var j=n(c);j!==null&&En(v,j.startTime-x)}}function k(x,j){w=!1,S&&(S=!1,f(P),P=-1),y=!0;var z=h;try{for(d(j),m=n(a);m!==null&&(!(m.expirationTime>j)||x&&!T());){var J=m.callback;if(typeof J=="function"){m.callback=null,h=m.priorityLevel;var ne=J(m.expirationTime<=j);j=e.unstable_now(),typeof ne=="function"?m.callback=ne:m===n(a)&&r(a),d(j)}else r(a);m=n(a)}if(m!==null)var gr=!0;else{var It=n(c);It!==null&&En(v,It.startTime-j),gr=!1}return gr}finally{m=null,h=z,y=!1}}var I=!1,N=null,P=-1,V=5,L=-1;function T(){return!(e.unstable_now()-L<V)}function A(){if(N!==null){var x=e.unstable_now();L=x;var j=!0;try{j=N(!0,x)}finally{j?ae():(I=!1,N=null)}}else I=!1}var ae;if(typeof s=="function")ae=function(){s(A)};else if(typeof MessageChannel<"u"){var H=new MessageChannel,K=H.port2;H.port1.onmessage=A,ae=function(){K.postMessage(null)}}else ae=function(){O(A,0)};function lt(x){N=x,I||(I=!0,ae())}function En(x,j){P=O(function(){x(e.unstable_now())},j)}e.unstable_IdlePriority=5,e.unstable_ImmediatePriority=1,e.unstable_LowPriority=4,e.unstable_NormalPriority=3,e.unstable_Profiling=null,e.unstable_UserBlockingPriority=2,e.unstable_cancelCallback=function(x){x.callback=null},e.unstable_continueExecution=function(){w||y||(w=!0,lt(k))},e.unstable_forceFrameRate=function(x){0>x||125<x?console.error("forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported"):V=0<x?Math.floor(1e3/x):5},e.unstable_getCurrentPriorityLevel=function(){return h},e.unstable_getFirstCallbackNode=function(){return n(a)},e.unstable_next=function(x){switch(h){case 1:case 2:case 3:var j=3;break;default:j=h}var z=h;h=j;try{return x()}finally{h=z}},e.unstable_pauseExecution=function(){},e.unstable_requestPaint=function(){},e.unstable_runWithPriority=function(x,j){switch(x){case 1:case 2:case 3:case 4:case 5:break;default:x=3}var z=h;h=x;try{return j()}finally{h=z}},e.unstable_scheduleCallback=function(x,j,z){var J=e.unstable_now();switch(typeof z=="object"&&z!==null?(z=z.delay,z=typeof z=="number"&&0<z?J+z:J):z=J,x){case 1:var ne=-1;break;case 2:ne=250;break;case 5:ne=1073741823;break;case 4:ne=1e4;break;default:ne=5e3}return ne=z+ne,x={id:p++,callback:j,priorityLevel:x,startTime:z,expirationTime:ne,sortIndex:-1},z>J?(x.sortIndex=z,t(c,x),n(a)===null&&x===n(c)&&(S?(f(P),P=-1):S=!0,En(v,z-J))):(x.sortIndex=ne,t(a,x),w||y||(w=!0,lt(k))),x},e.unstable_shouldYield=T,e.unstable_wrapCallback=function(x){var j=h;return function(){var z=h;h=j;try{return x.apply(this,arguments)}finally{h=z}}}})(ls);is.exports=ls;var cd=is.exports;/**
 * @license React
